...
void updateScoreDisplay() {
  ...
  String gamePrefix = (gameVariant == BONK_CHICKEN) ? "BONK! " : 
                     (gameVariant == BONK_BUNNY) ? "HOP! " : "FLAG! ";
  ...
  if (Wire.available()) {
    ...
    if (data[2] > 0) {
      ...
      if (currentTime - lastTapTime > debounceDelay) {
        ...
        switch (currentState) {
          case GAME_SELECT:
            // Handle game variant selection (three options now)
            if (x > 30 && x < 170 && y > 120 && y < 220) {
            ...
            else if (x > 180 && x < 320 && y > 120 && y < 220) {
            ...
            else if (x > 330 && x < 470 && y > 120 && y < 220) {
...
// Updated function to handle three game variant options
void handleGameSelection() {
  unsigned long currentTime = millis();
  ...
  if ((currentTime - stateStartTime) < 100 || 
    ...
    // BONK Chicken option (smaller boxes for three options)
    tft.drawRect(30, 120, 140, 100, YELLOW);
    tft.drawRect(31, 121, 138, 98, YELLOW);
    tft.fillRect(32, 122, 136, 96, RED);
    tft.setTextColor(WHITE);
    tft.setTextSize(2);
    ...
    // BONK Bunny option
    tft.drawRect(180, 120, 140, 100, WHITE);
    tft.drawRect(181, 121, 138, 98, WHITE);
    tft.fillRect(182, 122, 136, 96, PURPLE);
    tft.setTextColor(WHITE);
    tft.setTextSize(2);
    ...
    // BONK Patriots option (NEW!)
    tft.drawRect(330, 120, 140, 100, BLUE);
    tft.drawRect(331, 121, 138, 98, BLUE);
    tft.fillRect(332, 122, 136, 96, RED);
    tft.setTextColor(WHITE);
    tft.setTextSize(2);
    ...
    tft.println("Classic Action!");
    
    tft.setTextColor(CYAN);
    tft.setCursor(190, 230);
    tft.println("Easter Edition!");
    
    tft.setTextColor(BLUE);
    tft.setCursor(345, 230);
    tft.println("Patriotic Fun!");
  }
}
...
void handlePressStartP1() {
  ...
  if ((currentTime - stateStartTime) < 100 || 
    ...
    String variantName = (gameVariant == BONK_CHICKEN) ? "CHICKEN" : 
                        (gameVariant == BONK_BUNNY) ? "BUNNY" : "PATRIOTS";
